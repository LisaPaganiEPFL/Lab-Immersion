...
def fixation_probability_stat(in_numbers, folder, migration_matrix, fitnesses, nb_sim, nb_cycles, growth_factor, Nmin, size_follow_numbers=10000, print_frequency=1, save_dynamics=False, dilution_std=None, fitness_range=None, structure = None):
    #Counter for fixation trajectories
    fix_count=0
    ...
    if fix_count>0:
        average_fixation_cycle = np.sum(fix_cycle)/fix_count
        std_fixation_cycle = np.std(fix_cycle[:fix_count])
        std_error_fixation_cycle = std_fixation_cycle/np.sqrt(fix_count)
    else :
        average_fixation_cycle = 0
        std_fixation_cycle = 0
        std_error_fixation_cycle = 0
        
    if ex_count>0:
        average_extinction_cycle = np.sum(ex_cycle)/ex_count
        std_extinction_cycle = np.std(ex_cycle[:ex_count])
        std_error_extinction_cycle = std_extinction_cycle/np.sqrt(ex_count)
    else:
        average_extinction_cycle = 0
        std_extinction_cycle = 0
        std_error_extinction_cycle = 0
    ...
    return average_extinction_cycle, average_fixation_cycle, proba, std_extinction_cycle, std_fixation_cycle, std_error_extinction_cycle,              std_error_fixation_cycle, fix_cycle[:fix_count], ex_cycle[:ex_count]
